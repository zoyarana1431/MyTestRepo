...
import { parseTestScenarioBody } from "@/lib/test-case-presentation";
import {
  PRIORITY,
  SEVERITY,
  TEST_CASE_STATUS,
...
export default function EditTestCasePage() {
  ...
  const detailHref = `/projects/${projectId}/test-cases/${testCaseId}`;

  const applyDetail = useCallback((d: TestCaseDetail) => {
    const { title: t, scenarioLine: sc } = parseTestScenarioBody(d.test_scenario);
    setTitle(t);
    setScenarioLine(sc);
  ...
  }, [projectId, testCaseId, applyDetail]);
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const updated = await apiFetch<TestCaseDetail>(`/api/v1/projects/${projectId}/test-cases/${testCaseId}`, {
        method: "PATCH",
        json: {
      ...
      <div className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm sm:p-8">
        <header className="flex gap-3 border-b border-slate-100 pb-6">
          <Link
            ...
            aria-label="Back to test case"
          >
            <BackIcon />
          </Link>
          <div>
            <h1 className="text-xl font-bold text-slate-900">Edit Test Case</h1>
        ...
        <form onSubmit={(e) => void onSubmit(e)} className="mt-8 space-y-10">
          ...
          <section className="border-t border-slate-100 pt-8">
            ...
            <div className="space-y-4">
              {steps.map((st, i) => (
                <div key={`${st.id ?? "n"}-${i}`} className="rounded-xl border border-slate-200 bg-slate-50/50 p-4">
